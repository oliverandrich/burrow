...
- **`NavItem.Icon` and `NavLink.Icon` are now `string`** (a template-define name) instead of `template.HTML`. Layouts render icons via the new `{{ icon .Icon }}` template function. Move icon SVGs into `templates/icons.html` with `{{ define "<app>/icon_<name>" }}` blocks and set `Icon: "<app>/icon_<name>"`. The new convention keeps each contrib's icons in one file, also reachable from the contrib's own templates via `{{ template "<app>/icon_<name>" . }}`. `contrib/admin.DashboardItem.Icon` follows suit. See `docs/guide/navigation.md`.
- **`HasDocuments.Documents()` returns `[]document.Document` instead of `[]any`.** Den 0.13.1 tightened `den.Register` to take `...document.Document`; burrow's interface follows so the document-type contract is enforced at compile time end-to-end. Types embedding `document.Base` satisfy the marker automatically — no source change for normal call sites that already pass `&User{}` / `&Note{}` etc. Random structs (which Den would have rejected at runtime) now fail at compile time. Migration: replace `return []any{&Foo{}, &Bar{}}` with `return []document.Document{&Foo{}, &Bar{}}` and add the `github.com/oliverandrich/den/document` import.
- **Silent template-func stubs removed; contrib `Dependencies()` reflect real coupling.** The core base FuncMap no longer stubs `csrfToken` / `csrfField` / `csrfHxHeaders` / `lang`; templates using them without the providing contrib now fail at parse time instead of rendering empty. `auth`, `admin`, and `jobs` `Dependencies()` updated to declare the real hard deps — see `docs/contrib/{auth,admin,jobs}.md` for the new lists. Host apps that registered `auth.New()` without `csrf` or `staticfiles` will now fail at startup; register the missing contribs.
- **`auth.DefaultAuthLayout()` returns `"auth/layout"` instead of `""`.** `contrib/auth` now ships its own Tailwind-styled, navbar-less default layout for public auth pages. Hosts that previously relied on the empty-string default (auth pages inheriting `srv.SetLayout`'s layout, which leaks the host's navbar into login pages) get the new clean look automatically. Hosts that had set a custom `auth.WithAuthLayout("...")` keep that behaviour. The shipped layout links `{{ staticURL "app/app.min.css" }}` — Pattern B convention; hosts that follow a different convention override via `WithAuthLayout`.
...
- **`example/hello` migrated to Tailwind v4.** Ships its own `tailwind.css` source; run via `mise run example-hello`, which builds the Tailwind CSS first and then starts the server. The generated `static/app.min.css` is gitignored — every invocation rebuilds it fresh, so no drift-check is needed.
- **`{{ icon "name" }}` template function** in the core FuncMap. Looks up the named template define and returns its rendered HTML, used by layouts to render `NavItem.Icon` / `NavLink.Icon` / `admin.DashboardItem.Icon` dynamically. Returns empty for unknown or empty names. Necessary because Go's built-in `{{ template "name" . }}` action requires a string literal at parse time.

### Removed

- **`contrib/bsicons` and the `RegisterIconFunc` plumbing deleted.** With no contrib still pulling the package in, the ~5000-line generated Bootstrap Icons wrapper, the matching `update-icons` mise recipe, and the `AppConfig.RegisterIconFunc` / `IconFuncs` / `IconFunc` API surface are all gone. Apps that need icons inline the SVG as a `{{ define "<app>/icon_*" }}` block in `templates/icons.html` and reference it by name from both templates and `NavItem.Icon` (see `contrib/jobs`, `contrib/auth`, `example/notes/internal/notes`).
- **`contrib/mucss` and `contrib/bootstrap` deleted.** Both were the previous design-contrib options; every in-tree consumer (contrib/admin, contrib/auth, contrib/jobs, example/notes, example/hello, tutorial/step03..08) is now Tailwind-based — except the tutorial, which ships a small hand-written `app.css` to keep the toolchain out of the way for learners. The µCSS/Bootstrap CSS bundles, `mu.css` accent variants, Sass theme sources, and `update-mucss` / `update-bootstrap` / `sass` mise tasks went with them.
- **`example/themes`** — was a µCSS accent-variant preview app, no longer meaningful after the framework drops µCSS.

### Changed

- **`contrib/admin` migrated to Tailwind v4.** The admin layout and dashboard templates drop the µCSS class system, the bsicons `iconPersonCircle` template func, and the theme-switcher dropdown; the layout now references the host app's `app/app.min.css` (Pattern B convention) and inlines the person-circle SVG. `Dependencies()` drops `"mucss"`. The `contrib/admin/static/admin.css` legacy file shrinks to just the `admin-row-actions` rules still consumed by auth/jobs/example-notes admin pages until their own migration beans land. No public Go API change.
- **`contrib/jobs` admin templates migrated to Tailwind v4.** `admin_list.html` (table + filter tabs + per-row action buttons + pagination) and `admin_detail.html` (info card + payload/result/error cards + action buttons) rewritten with Tailwind utilities; `jobs/status_badge` shows level-aware coloured pills. Three icons (`iconArrowCounterclockwise`, `iconXCircle`, `iconTrash`) and the nav-item icon `bsicons.ListTask()` inlined as SVG; bsicons import dropped. Pagination markup inlined as `jobs/pagination` template so jobs no longer depends on `mucss/pagination`. No public Go API change.
- **`contrib/auth` templates migrated to Tailwind v4.** All ten templates (public flow: `login`, `register`, `recovery`, `recovery_codes`, `verify_pending`/`success`/`error`, `credentials`, `centered`/`card`/`logo`/`csrf_input`; admin: `admin_users`, `admin_user_form`, `admin_invites`, `admin_invite_form`) drop µCSS classes for Tailwind utilities. The `<style>` block in `shared.html` is gone — `auth/centered` and `auth/card` are Tailwind containers. Element IDs consumed by `webauthn.js` (`csrf-token`, `login-btn`, `register-form`, `cred-{ID}`, ...) are byte-identical. Five action-icon defines (`auth/icon_plus_lg`, `_trash`, `_person_slash`, `_person_check`, `_x_circle`) live in `templates/icons.html`; `AdminNavItems` uses two inline-SVG consts (`peopleIcon`, `envelopeIcon`). The `bsicons` import and all six `cfg.RegisterIconFunc` registrations are gone (the registered `iconSearch` was unused).
- **`DefaultAuthLayout` returns `""` instead of `"mucss/layout"`.** Auth public routes now inherit the global layout set by the host via `srv.SetLayout`, removing the hard µCSS coupling. Hosts that want a dedicated layout for unauthenticated pages continue to pass `auth.WithAuthLayout("...")`.
- **`example/notes` migrated to Tailwind v4.** All four template sets (`pages/home`, `notes/list`, `notes/form`, `notes/admin_list`) and the shell layout drop µCSS classes and bsicons template funcs for Tailwind utilities and inline `<svg>` icons defined as `app/icon_*` templates in the layout. The `internal/pages` package was renamed to `internal/app` to match the Pattern B convention from `docs/guide/tailwind.md` and now implements `HasStaticFiles` so its `static/app.min.css` is served under `/static/app/...`. The theme-switcher dropdown is gone — dark mode follows `prefers-color-scheme` only. `cmd/server/main.go` drops the `contrib/mucss` registration.
- **`cmd/burrow-tailwind` writes `.tailwind/sources.css` next to the input `tailwind.css`** (parsed from `-i`/`--input`) instead of relative to the working directory, so calling the tool from the repo root with `-i example/<app>/tailwind.css` now puts the sources file where the `@import "./.tailwind/sources.css"` line inside `tailwind.css` actually resolves it. Falls back to cwd-relative when `-i` is not provided.
...
- **`docs/guide/testing.md` rewritten against the current `burrowtest` API.** `burrow.TestDB` / `burrow.TestErrorExecContext` / `burrow.TestErrorExecMiddleware` references are replaced with `burrowtest.DB` + explicit `den.Register`, `burrowtest.ErrorExecContext`, `burrowtest.ErrorExecMiddleware`. Adds sections for `burrowtest.TempDSN` and `burrowtest.StubApp`; corrects `authtest.NewDB` / `authtest.WithID` signatures.
- **`docs/reference/server.md` boot sequence rewritten** to match the actual order in `Server.Run` (i18n bundle is created in step 4, not step 7; PostConfigure runs inside `Registry.Configure` as the second of two phases, not as a separate top-level step). Adds reference sections for `Server.CLICommands()` and `Server.TemplateExecutor()`; documents `Registry.Configure` (canonical, two-phase) and clarifies `Registry.ConfigureAll` is a test-only convenience.
- **`docs/reference/core-functions.md` gains a `burrow.OpenDB` reference.** Documents the Den-backend blank-import requirement at the function level (previously only mentioned in passing in `configuration.md`) and points test code at `burrowtest.DB`. `docs/getting-started/installation.md`'s verify snippet gains the SQLite blank-import; `docs/contrib/messages.md`'s interface table gains the missing `HasRequestFuncMap` row.
- **Tutorial parts 1-8 prose aligned with the actual `tutorial/stepNN/` source.** Notable fixes: Part 1's main.go gets the SQLite blank-import; Part 2's `Question` struct + repository match step02 (PublishedAt indexed, `Choices []Choice` field, ptr-to-value conversion); Part 4 drops htmx in favour of healthcheck (matches step04), uses `den.Save` + `MaxBytesReader`; Part 5 stops inventing a non-existent `Layout()` data-plumbing method (auth's `HasRequestFuncMap` already provides `currentUser`); Part 6 adds a "Wire the Auth CLI Subcommands" section and fixes the `promote` syntax to a positional argument; Part 7 re-registers htmx, fixes `--mu-primary` to `--accent`, `%d` to `%s` for the URLParam, and rewrites `ListQuestionsPaged` to use `AllWithCount` sorted by `_id`; Part 8 reframes the pagination tip to explain why the tutorial keeps a local `polls/pagination` define. `tutorial/step06`, `step07`, `step08` `main.go` gain `Commands: srv.CLICommands()` so the `promote` subcommand actually works; all eight tutorial `go.mod` files refreshed via `go mod tidy`.

### Fixed

- **`admin/pagination` template lifted into `contrib/admin`.** The three near-identical `{{ define "<app>/pagination" }}` blocks (`jobs/pagination`, `auth/pagination`, `notes/pagination`) — same 26-line Tailwind markup, just different define names — collapsed into a single canonical `admin/pagination` shipped by `contrib/admin/templates/pagination.html`. The four call sites (`contrib/jobs/templates/admin_list.html`, `contrib/auth/templates/admin_users.html` + `admin_invites.html`, `example/notes/internal/notes/templates/admin_list.html`) now invoke `{{ template "admin/pagination" ... }}` with the same `(dict "Page" .Page "BasePath" "..." "RawQuery" .RawQuery)` contract as before. Downstream apps that override `jobs/pagination` / `auth/pagination` / `notes/pagination` must switch to overriding `admin/pagination` instead. `docs/guide/pagination.md`, `docs/tutorial/part8.md`, and `docs/reference/reusable-templates.md` updated from the obsolete `mucss/pagination` reference (left over from the v0.18 mucss removal) to the new canonical name.
- **`tutorial/step03..08` migrated off mucss.** The `internal/pages` package was renamed to `internal/app` (matching the docs convention used by `example/notes`) and now ships a small hand-written `app.css` via `HasStaticFiles` instead of depending on the deleted `contrib/mucss`. Each step's `main.go` drops the `mucss.New()` registration; the layout templates link `{{ staticURL "app/app.css" }}` instead of pulling in `mucss/css` + `mucss/theme_script` + `mucss/theme_switcher`. Tutorial parts 2-8 prose updated to match. `step08` keeps a local `polls/pagination` define (uses our stylesheet's `.pagination` rule) with a tip pointing at the shared `admin/pagination` template for projects that ship Tailwind.
...
- **`{{ icon "..." }}` no longer poisons the global template tree.** `Server` now keeps a separate `iconTemplates` Clone for icon execution so executing icon defines doesn't mark the main `templates` tree as "executed". Before the fix, any page that rendered an icon (admin dashboard, nav links with `Icon` set, …) followed by a layout wrap returned `HTTP 500: html/template: cannot Clone "" after it has executed`. Regression test: `TestExecuteTemplateUsingIconDoesNotPoisonClone`.
- **`example/notes` no longer renders the empty-state placeholder alongside created notes.** `notes/create_response` OOB-deletes the `#notes-empty` placeholder when prepending the first note, and a new `notes/delete_response` re-injects the placeholder when the last note is deleted. `Repository.CountByUserID` added to support the delete-time check.
