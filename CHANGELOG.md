...
- **`contrib/admin` migrated from Bootstrap to µCSS + redesigned without sidebar** — `layout.html`, `index.html` and the admin-area templates in `contrib/auth` (`admin_users`, `admin_user_form`, `admin_invites`, `admin_invite_form`), `contrib/jobs` (`admin_list`, `admin_detail`, `status_badge`) and `example/notes` (`admin_list`) rewritten using native µCSS classes — no inline `<style>` blocks, no `style=""` attributes, no admin-specific stylesheet. The collapsible sidebar pattern was dropped entirely: admin now uses a top-nav layout (brand on the left, user-menu + theme switcher on the right via `<details class="dropdown">`), and the dashboard at `/admin` is the primary navigation surface (cards grid via `.container-fluid` + `.row` + `.col-{12,sm-6,md-4}`). Each admin page renders its own breadcrumb (`<nav aria-label="breadcrumb"><ul class="breadcrumb">…<li aria-current="page">`) for back-navigation. Filter pills use `<ul class="tabs tabs-pills">` with `aria-selected="true"`, badges use `.badge-{primary,secondary,success,warning,error,info}`, buttons use `.btn .btn-outline .btn-{secondary,success,error,warning}`. **Public-API breaks:** `SidebarGroup`/`SidebarItem`/`PrepareSidebar` renamed to `DashboardGroup`/`DashboardItem`/`PrepareDashboard`; the `Active bool` field is gone (no sidebar = no active-link state); `sidebarLinkClass` and `isActivePath` removed; the `adminSidebar` template func renamed to `adminDashboard`. `contrib/admin` now declares `staticfiles`, `htmx`, `mucss`, and `messages` as Dependencies and ships a single `static/admin.css` (a handful of admin-shell polish rules: action-row alignment, dashboard card link styling, form-footer flex). Apps with custom admin extensions that emitted Bootstrap markup must update.
...
- **Documentation now leads with µCSS as the default design contrib.** README features list, `docs/getting-started/{examples,tooling,project-structure,coming-from-django}.md`, `docs/contrib/admin.md`, `docs/tutorial/part6.md`, `docs/reference/context-helpers.md`, and `docs/contrib/secure.md` updated to describe the µCSS-first state. CLAUDE.md retires the "CSS framework default contrib unsettled" Open Question and records the resolved decision under Decisions Made (default = µCSS, Bootstrap deprecated for removal in v0.20). `mkdocs.yml` already lists µCSS first under "UI & Templates"; no ordering change needed.
- **Tutorial gains a Part 8: Custom Admin Views.** New `tutorial/step08` snapshot extends the polls app with `burrow.HasAdmin` (`AdminRoutes` + `AdminNavItems`), full Question/Choice CRUD admin handlers, supporting Repository methods (`SearchQuestionsPaged`, `UpdateQuestion`, `DeleteQuestion` with manual cascade, `UpdateChoice`, `DeleteChoice`, `GetChoice`), and two new admin templates (`admin_list.html`, `admin_form.html`). New `docs/tutorial/part8.md` explains the `HasAdmin` interface and walks through the implementation; `docs/tutorial/index.md`, `mkdocs.yml` nav, and `part7.md`'s "Next" section updated to point to Part 8.
- **`tutorial/step03`-`step07`** migrated from Bootstrap to µCSS. Each step's templates (layout, home, polls/list, polls/detail, polls/results, plus step07's question_item + list_page fragments) now use native µCSS classes (`<article>`, `<table>`, `.btn`, `.alert-{level}`, `.badge-primary`, `.hero`, container/row/col grid, `<section class="hero">` for the landing page). Each step's `main.go` registers `mucss.New()` instead of `bootstrap.New()`. The user menu evolves with the tutorial: step05/06 use a simple inline `<form>` + button for sign-out (CSRF via `{{ csrfField }}`); step07 — once htmx is loaded — refactors it into a `<details class="dropdown">` with an `<a hx-post="/auth/logout">` link, using `csrfHxHeaders` on `<body>` so the HTMX request gets the CSRF token via header. The polls app now implements `Seedable` with a `Seed(ctx)` method that inserts a few example questions when the server is started with `--seed`, plus `Repository.CreateQuestion`/`CreateChoice` helpers. The accompanying tutorial documentation (`docs/tutorial/part3.md`-`part7.md`) was updated to mirror the new templates and explain the µCSS conventions used.

### Deprecated

- **`contrib/bootstrap`** is deprecated and scheduled for removal in v0.20. After the µCSS migration epic landed, no in-tree app uses Bootstrap anymore (admin, auth user-facing, examples, tutorial all on µCSS). The package will continue to receive critical fixes only — no new features. New projects should use [`contrib/mucss`](https://burrow.readthedocs.io/contrib/mucss/) (the conversion is mostly mechanical: `<div class="card">` → `<article>`, `alert-danger` → `alert-error`, `<button class="btn btn-primary">` keeps the same class names). `contrib/bsicons` (inline-SVG template functions, not Bootstrap-coupled) stays.

### Added

- **`contrib/mucss` ships Burrow UX extras** — a small `mu-extras.min.css` is now always loaded alongside the main stylesheet (disabled when `WithCustomCSS` is set). Currently includes: `min-width: 8rem` on navbar dropdowns so single-word menus don't look orphaned; `vertical-align: middle` on navbar SVG icons (overrides bsicons' default typographic dip) plus a small inline-end gutter on icons inside `<a>`/`<button>`/`<summary>`; a `display: flex` layout for `dialog > article > header` so close icons and titles vertically center cleanly (replaces upstream µCSS's `float:right` which never aligned reliably); a `display: flex` layout for form action footers inside dialogs so Save/Cancel pairs sit inline with content-sized buttons (Pico/µCSS's `button[type=submit]{width:100%}` default doesn't suit inline action rows); and a `.field-error` rule for inline form-field error messages. The `mucss/theme_switcher` template uses `<ul dir="rtl">` + `<li dir="ltr">` to right-anchor the popup while keeping per-item content in natural left-to-right reading order.
- **HTMX-driven dialog pattern** — `htmx.OpenDialog(w, id)` and `htmx.CloseDialog(w, id)` helpers emit `HX-Trigger-After-Swap` events that the new `htmx/dialog_script` template translates into `dialog.showModal()` / `dialog.close()` on the client. `OpenDialog` accepts an optional third argument that replaces the dialog element's class before opening — `htmx.OpenDialog(w, "modal", "modal-lg")` for µCSS size variants (`modal-sm`/`modal-lg`/`modal-fullscreen`) that target the dialog rather than its inner content. The `mucss` and `bootstrap` `nav_layout` templates now ship a permanent empty `<dialog id="modal"></dialog>` container and include the script automatically. Each view renders its own `<article>` as the swapped content; HTMX targets `#modal` with `innerHTML`. Forms posted into a dialog re-render in place on validation errors; successful responses close the dialog and update other parts of the page via OOB swaps. Clicking any element with `rel="prev"` (µCSS close-icon button convention with attached styling) or the behavioral `data-close-dialog` attribute inside an open dialog closes it client-side. See `example/notes` for the full create + edit + cancel flow.
