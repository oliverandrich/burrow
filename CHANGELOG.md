...
- **`contrib/mucss` ships Burrow UX extras** — a small `mu-extras.min.css` is now always loaded alongside the main stylesheet (disabled when `WithCustomCSS` is set). Currently sets `min-width: 8rem` on navbar dropdowns so single-word menus don't look orphaned. The `mucss/theme_switcher` template uses `<ul dir="rtl">` + `<li dir="ltr">` to right-anchor the popup while keeping per-item content in natural left-to-right reading order.
- **HTMX-driven dialog pattern** — `htmx.OpenDialog(w, id)` and `htmx.CloseDialog(w, id)` helpers emit `HX-Trigger-After-Swap` events that the new `htmx/dialog_script` template translates into `dialog.showModal()` / `dialog.close()` on the client. The `mucss` and `bootstrap` `nav_layout` templates now ship a permanent `<dialog id="modal"><div id="modal-body"></div></dialog>` container and include the script automatically. Forms posted into a dialog re-render in place on validation errors; successful responses close the dialog and update other parts of the page via OOB swaps. Clicking any element with `rel="prev"` inside an open dialog closes it client-side. See `example/notes` for the full create + edit + cancel flow.
