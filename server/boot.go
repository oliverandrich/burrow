...
func registerApp(reg *registry.Registry, app burrowapp.App) {
...
func logCapabilities(app burrowapp.App) {
	var caps []string
	if _, ok := app.(burrowapp.HasDocuments); ok {
		caps = append(caps, "documents")
	}
	if _, ok := app.(burrowapp.HasRoutes); ok {
		caps = append(caps, "routes")
	}
	if _, ok := app.(burrowapp.HasMiddleware); ok {
		caps = append(caps, "middleware")
	}
	if _, ok := app.(burrowapp.HasNavItems); ok {
		caps = append(caps, "nav")
	}
	if _, ok := app.(burrowapp.HasFlags); ok {
		caps = append(caps, "flags")
	}
	if _, ok := app.(burrowapp.Configurable); ok {
		caps = append(caps, "config")
	}
	if _, ok := app.(burrowapp.HasCLICommands); ok {
		caps = append(caps, "commands")
	}
	if _, ok := app.(burrowapp.HasMigrations); ok {
		caps = append(caps, "migrations")
	}
	if _, ok := app.(burrowapp.HasAdmin); ok {
		caps = append(caps, "admin")
	}
	if _, ok := app.(burrowapp.HasStaticFiles); ok {
		caps = append(caps, "staticfiles")
	}
	if _, ok := app.(burrowapp.HasTranslations); ok {
		caps = append(caps, "translations")
	}
	if _, ok := app.(registry.HasDependencies); ok {
		caps = append(caps, "dependencies")
	}
	if _, ok := app.(burrowapp.HasShutdown); ok {
		caps = append(caps, "shutdown")
	}
	if _, ok := app.(burrowapp.ReadinessChecker); ok {
		caps = append(caps, "readiness")
	}
	if _, ok := app.(burrowapp.HasTemplates); ok {
		caps = append(caps, "templates")
	}
	if _, ok := app.(burrowapp.HasFuncMap); ok {
		caps = append(caps, "funcmap")
	}
	if _, ok := app.(burrowapp.HasRequestFuncMap); ok {
		caps = append(caps, "requestfuncmap")
	}
	if _, ok := app.(tasks.HasJobs); ok {
		caps = append(caps, "jobs")
	}
	if _, ok := app.(burrowapp.PostConfigurable); ok {
...
func configureAll(reg *registry.Registry, cfg *burrowapp.AppConfig) error {
	for _, app := range registry.Apps(reg) {
		if provider, ok := app.(burrowapp.Configurable); ok {
...
func configure(reg *registry.Registry, cfg *burrowapp.AppConfig, cmd *cli.Command) error {
	...
	for _, app := range apps {
		if provider, ok := app.(burrowapp.PostConfigurable); ok {
...
func registerMiddleware(reg *registry.Registry, router chi.Router) {
	for _, app := range registry.Apps(reg) {
		if provider, ok := app.(burrowapp.HasMiddleware); ok {
...
func registerRoutes(reg *registry.Registry, router chi.Router) {
	for _, app := range registry.Apps(reg) {
		if provider, ok := app.(burrowapp.HasRoutes); ok {
...
func runMigrations(ctx context.Context, reg *registry.Registry, db *den.DB) error {
	mreg := migrate.NewRegistry()
	for _, app := range registry.Apps(reg) {
		provider, ok := app.(burrowapp.HasMigrations)
		if !ok {
			continue
		}
...
func registerDocuments(ctx context.Context, reg *registry.Registry, db *den.DB) error {
	for _, app := range registry.Apps(reg) {
		hd, ok := app.(burrowapp.HasDocuments)
		if !ok {
			continue
		}
...
func allFlags(reg *registry.Registry, configSource func(key string) cli.ValueSource) []cli.Flag {
	var flags []cli.Flag
	for _, app := range registry.Apps(reg) {
		if provider, ok := app.(burrowapp.HasFlags); ok {
...
func allNavItems(reg *registry.Registry) []burrowapp.NavItem {
	var items []burrowapp.NavItem
	for _, app := range registry.Apps(reg) {
		if provider, ok := app.(burrowapp.HasNavItems); ok {
...
// sorted by Position.
func allAdminNavItems(reg *registry.Registry) []burrowapp.NavItem {
	var items []burrowapp.NavItem
	for _, app := range registry.Apps(reg) {
		if provider, ok := app.(burrowapp.HasAdmin); ok {
...
func allCLICommands(reg *registry.Registry) []*cli.Command {
	var cmds []*cli.Command
	for _, app := range registry.Apps(reg) {
		if provider, ok := app.(burrowapp.HasCLICommands); ok {
...
func shutdownApps(ctx context.Context, reg *registry.Registry) error {
	var errs []error
	apps := registry.Apps(reg)
	for _, v := range slices.Backward(apps) {
		if provider, ok := v.(burrowapp.HasShutdown); ok {
